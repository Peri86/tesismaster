...
def escollir(arllegit,nllegit,nomar):
    ...
    for i in range(len(sel)):
        if sel[i] == "infomap":
            ...
            if tr == '':
                ...
                t2 = time.time()
                print 'La funcio ha tardat %0.3f ms' % ((t2-t1)*1000.0)
            ...
            try:
                cfx = open("./cfinder/"+nomar+".dat_files/k=3/communities", "r")
            except IOError:
            ...
            p = subprocess.Popen("./commfind/commfind ./commfind/"+nomar+".graphml "+str(nuigen), shell=True)
            p.communicate()
...
def netescollir(arllegit,nllegit,nomar):
