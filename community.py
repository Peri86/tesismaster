...
class Status :
    ...
    def init(self, graph, part = None) :
        ...
        self.gdegrees = dict([])
        self.internals = dict([])
        self.total_weight = graph.size(weight='weight') #CANVIO A weight='weight' abans True
        if part == None :
            for node in graph.nodes() :
                self.node2com[node] = count
                deg = float(graph.degree(node, weight='weight')) #DONA ERROR canvio a weight='weight' abans True
            ...
            for node in graph.nodes() :
                ...
                deg = float(graph.degree(node, weight='weight')) #CANVIO A weight='weight' abans True
